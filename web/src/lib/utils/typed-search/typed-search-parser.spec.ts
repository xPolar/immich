...
describe('parseTypedSearch', () => {
  ...
  it('keeps Windows paths in the plain query', () => {
    const result = parseTypedSearch(String.raw`C:\photos\trip.jpg type:image`);

    expect(result.queryText).toBe(String.raw`C:\photos\trip.jpg`);
    expect(result.scalarTokens).toMatchObject([{ key: 'type', normalizedValue: 'image' }]);
    expect(result.issues).toEqual([]);
  });
